...
Nodes:
  ...
  P3 select_dataset     [Code + interrupt]
  P4 read_schema        [Code]
...
def _parse_available_list(text: str) -> list[dict[str, Any]]:
    """Extract table names from SearchDatasetsTool error message."""
    # Format: "Dataset 'X' not found. Available datasets: a, b, c"
    if "Available datasets:" in text:
        _, _, after = text.partition("Available datasets:")
...
) -> Command[Literal["search_dataset"]]:
    prompt = PARSE_PROMPT.format(request=state["request"][:500])
...
def search_dataset(
    state: DashboardState,
) -> Command[Literal["select_dataset"]]:
    ...
    # Exact match: JSON response with dataset metadata
    if result_str.startswith("{"):
        try:
            dataset = json.loads(result_str)
        except json.JSONDecodeError:
            dataset = {"table_name": target}
        return Command(
            update={"dataset_candidates": [dataset]},
            goto="select_dataset",
        )

    # Fallback: parse available list
    candidates = _parse_available_list(result_str)
    return Command(
...
# ── Node P3: select_dataset [Code + interrupt] ─────────────────────
...
def select_dataset(  # noqa: C901
    ...
    # Always auto-select the best match (interrupt requires checkpointer
